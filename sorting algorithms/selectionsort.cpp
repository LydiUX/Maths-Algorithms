...
/*
selection sort:
1. keep a pointer on the first element
2. traverse the rest of array to find the lowest value
3. swap with the pointer
4. increase pointer location by one
...
int main(){
    ...
    clock_t t = clock();
    selectionSort(arr, n - 1); //n - 1 as arr[n] is out of index (starts at index 0)
    t = clock() - t;
