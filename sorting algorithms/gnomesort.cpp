...
/*
gnome sort:
1. start at the second index of array
2. check if the element you are on and the previous element are sorted
3. if yes, increase index by one; if no, swap the two and decrease index by one
...
int main(){
    ...
    clock_t t = clock();
    gnomeSort(arr, n - 1); //n - 1 as arr[n] is out of index (starts at index 0)
    t = clock() - t;
